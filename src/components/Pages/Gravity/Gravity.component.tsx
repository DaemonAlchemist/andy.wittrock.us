...
const GravityComponent = (props:IPageComponentProps) =>
    <>
        <p><DropCap preview={props.preview}>Y</DropCap>ears ago, when I was still in college and had lots of free time, I played around with physics simulations of various kinds.  One that I tried was a classic gravitational simulation.  It's great fun to setup complex arrangements of planets and stars, turn on the simulation, and watch star systems evolve.</p>

        {!props.preview && <>
            <h2>Simulating Collisions</h2>
            ...
            <h2>3D Sorting Algorithms</h2>

            <p>I've done some recent work with <a href="http://en.wikipedia.org/wiki/K-d_tree">k-dimensional trees</a> as a way to create a more efficient nearest neighbor algorithm.  This data structure is useful for organizing points in space so that you can search more efficiently in multiple dimensions.  Another similar data structure is the <a href="http://en.wikipedia.org/wiki/Octree">octree</a> which has a more regular structure than the kd-tree.</p>

            <p>Since my goal for these simulations was aesthetic rather than scientific, I can speed up force calculations by grouping all of the particles into an octree.  Since a group of points becomes more and more like a single point the further away they get, I can get the force from a particular octree node by averaging the positions of the particles inside the node and treating the node as a single particle.  Thus, the force from a large group of points far away from another point can be calculated with a single operation instead of many.</p>

            <p>Although complete accuracy isn't necessary for a purely visual effect, I don't want the simulation to be <em>too</em> unrealistic.  So, before I start implementing an octree-based simulation, I need to know how much &#8220;cheating&#8221; I can get away with.  To do this, I wrote a PHP script that creates 2x2x2 cubes of 10 random masses at various distances from the origin, and then calculates the magnitude and angle error of the averaged force compared to the actual force.</p>
